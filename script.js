...
function createCandle() {
	...
	var caseMat = new THREE.MeshStandardMaterial({ color: 0xEEEEEE }); // Orange-red color
	...
	const topMaterial = new THREE.MeshStandardMaterial({ color: 0xDDDDDD });
...
var globalLight = new THREE.PointLight(0xffaa33, 2, 5, 2);
globalLight.position.set(0, candleHeight + 1, 0);
...
var slashTime = 2 + Math.random() * 2; // 2-4 s 
var startTime = candleCount*3 + slashTime;
var endTime = startTime + 1.1;
...
function render() {
	...
	for(let i = 0 ; i < candles.length ; i++){
		let scale = (time - 3*i)*0.1;
		scale = Math.min(scale, 0.3);
		scale = Math.max(scale, 0);
		candles[i].scale.set(scale, scale, scale);
	}

	if(candles.length > 2){
		toggleCandle(candles[2], time > startTime && time < endTime);
	}
	controls.update();
...
// 將蠟燭添加到蛋糕上
cake.add(createCandles(candleCount));

candles.forEach(candle => toggleCandle(candle, false));
...
function toggleCandle(candle, on) {
	...
	flames.forEach(flame => flame.visible = on);
	lights.forEach(light => light.intensity = on ? 1.0 : 0.0);
}
